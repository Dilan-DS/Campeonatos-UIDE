...
def generar_fixture_eliminatoria(campeonato_id):
    ...
    print(f"Generando fixture de eliminatoria simple para el campeonato: {campeonato.nombre}")

    def create_knockout_matches_for_gender_group(team_list, championship, start_date, end_date, valid_days_of_week):
        num_teams = len(team_list)
        if num_teams < 2:
            print(f"No hay suficientes equipos en este grupo de género para generar partidos de eliminatoria.")
            return 0

        if not (num_teams > 0 and (num_teams & (num_teams - 1) == 0)): # Check if num_teams is a power of 2
            print(f"Advertencia: El número de equipos ({num_teams}) en este grupo de género no es una potencia de 2. Se necesitarán byes o ajustes para el formato de eliminatoria simple.")
    ...
    total_matches_created += create_knockout_matches_for_gender_group(equipos_masculinos, campeonato, campeonato.fecha_inicio, campeonato.fecha_fin, campeonato.dias_partido)
    total_matches_created += create_knockout_matches_for_gender_group(equipos_femeninos, campeonato, campeonato.fecha_inicio, campeonato.fecha_fin, campeonato.dias_partido)

    if total_matches_created == 0:
        print("No se pudieron crear partidos para la eliminatoria.")
    else:
        print(f"Fixture de eliminatoria simple generado para {campeonato.nombre}. Se crearon {total_matches_created} partidos.")
